var chartRT = function () {
  ...
  _self.DataSeries = [];
  _self.Ticks = 2;
  _self.TickDuration = 10000;
  // Moved to HTML File _self.MaxValue = 100;
  _self.w = 1000;
  _self.h = 800;
  _self.margin = { top: 50, right: 200, bottom: 60, left: 60 };
  ...
  _self.Init = function () {
    ...
    //
    // Back fill DataSeries with Previous Data
    //
    ...
    //  SVG Canvas
    _self.svg = d3.select("#graph").append("svg")
    ...
    .attr("y1", function (d) { return d * (_self.height / (_self.MaxValue / 5)); })
    .attr("y2", function (d) { return d * (_self.height / (_self.MaxValue / 5)); })
    .style("stroke", "#eee")
    .style("opacity", .05)
    .attr("clip-path", "url(#clip-" + _self.guid + ")")
    ...
    _self.tick = function (id) {
      ...
      for (i in _self.DataSeries) {
        ...
        //while (_self.DataSeries[i].Data.length <_self.Ticks ) {
